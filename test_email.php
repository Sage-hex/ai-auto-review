...
// Include the direct SMTP helper
require_once __DIR__ . '/backend/helpers/direct_smtp.php';
...
if (isset($_POST['send_test'])) {
    ...
    // Try sending via direct SMTP
    echo "<h2>Sending Test Email</h2>";
    echo "<p>Attempting to send email to: $to</p>";
    
    $result = sendDirectSMTP($to, $subject, $message);
    
    if ($result) {
        echo "<p style='color: green; font-weight: bold;'>Email sent successfully via direct SMTP!</p>";
    } else {
        echo "<p style='color: red; font-weight: bold;'>Failed to send email via direct SMTP.</p>";
        
        // Try using PHP's mail() function as a fallback
        echo "<p>Trying PHP mail() function as fallback...</p>";
        
        $headers = [
            'MIME-Version: 1.0',
            'Content-Type: text/html; charset=UTF-8',
            'From: AI Auto Review <' . getenv('MAIL_FROM_EMAIL') . '>',
        ...
        if ($mail_result) {
            echo "<p style='color: green; font-weight: bold;'>Email sent successfully via PHP mail() function!</p>";
        } else {
